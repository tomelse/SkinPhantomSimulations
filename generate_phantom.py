...
from spectra.melanin import melanosome_mu_a
from spectra.haemoglobin import blood_mu_a
import os
import glob
from itertools import product
import pandas as pd

experiment_folder = "Exp5"
...
for vol_num, vol_values in enumerate(volume_combinations):
    ...
    for physical_num, physical_values in enumerate(physical_combinations):
        ...
        mu_a_spectra = [lambda x: 0, lambda x: 1e-6, lambda x: blood_mu_a(x, haematocrit=bg_hmt),
                        lambda x: melanosome_mu_a(x, melanin_coeff), lambda x: blood_mu_a(x, SO2)]
...
files = glob.glob("Exp4/settings_*.json")
